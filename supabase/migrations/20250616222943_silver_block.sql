/*
  ...
  1. Problem
    - The `handle_auth_user_creation` function attempts to update primary key columns
    - This causes foreign key constraint violations
    - Results in "Database error querying schema" during authentication

  2. Solution
    - Simplify user creation logic to avoid primary key updates
    - Use proper conflict resolution strategies
    - Ensure admin setup doesn't cause constraint violations

  3. Changes
    - Fix `handle_auth_user_creation` function
    - Fix `ensure_admin_user_setup` function
    - Use safer conflict resolution patterns
*/
...
INSERT INTO configuracion (key, valor) VALUES
    ...
    ('primary_key_update_issue_resolved', 'true')
ON CONFLICT (key) DO UPDATE SET 
    valor = EXCLUDED.valor,
    updated_at = NOW();