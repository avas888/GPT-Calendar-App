/*
  ...
  1. Functions
    - `ensure_admin_user_setup()` - Creates admin user profile and role if needed
    - `handle_auth_user_creation()` - Links Supabase Auth users to app profiles
    - `is_admin_user()` - Helper function to check admin status without RLS recursion

  2. Security
    - Updates all RLS policies to use the helper function
    - Ensures admin users have full privileges across all tables
    - Fixes syntax errors in user creation logic

  3. Configuration
    - Sets up initial admin configuration
...
BEGIN
    ...
    ELSE
        ...
        -- Handle email conflict separately
        IF NOT FOUND THEN
            UPDATE usuarios 
            SET id = auth_user_id, nombre = user_name
            WHERE correo = auth_email;
...
INSERT INTO configuracion (key, valor) VALUES
    ...
    ('production_database_mode', 'enabled'),
...
COMMENT ON FUNCTION ensure_admin_user_setup IS 'Ensures admin user profile and role exist for production database access';
