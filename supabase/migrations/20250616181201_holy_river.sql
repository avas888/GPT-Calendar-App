/*
  # Admin User Setup and Policy Migration

  1. Functions
    - `ensure_admin_user_setup()` - Creates admin user and role if needed
    - `handle_auth_user_creation()` - Links auth users to profiles
    - `is_admin_user()` - Helper function to check admin status safely

  2. Security
    - Updates all RLS policies to use the helper function
    - Prevents recursion issues in policy evaluation
    - Ensures admin has full access to all tables

  3. Configuration
    - Sets up admin configuration values
...
BEGIN
    ...
    ELSIF existing_admin_id IS NULL AND admin_user_id IS NULL THEN
        ...
        -- Insert admin user profile (handle conflicts separately)
        ...
        -- Handle email conflict separately if needed
        IF NOT EXISTS (SELECT 1 FROM usuarios WHERE correo = admin_email) THEN
            INSERT INTO usuarios (id, correo, nombre)
            VALUES (admin_user_id, admin_email, 'Administrador')
...
INSERT INTO configuracion (key, valor) VALUES
    ...
    ('migration_fixed_syntax_error', 'true')
ON CONFLICT (key) DO UPDATE SET 
    valor = EXCLUDED.valor,
    updated_at = NOW();
