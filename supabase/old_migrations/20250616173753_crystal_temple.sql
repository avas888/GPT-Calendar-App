/*
  # Fix Admin User Setup with Maximum Credentials

  1. Database Functions
    - Create `ensure_admin_user_setup()` function for handling admin user creation
    - Create `handle_auth_user_creation()` function for linking auth users to profiles
    - Create `is_admin_user()` helper function to avoid RLS recursion

  2. Security Policies
    - Update all table policies to use recursion-safe admin checks
    - Ensure admin users have maximum credentials across all tables
    - Handle duplicate key constraints safely

  3. Configuration
    - Set admin email configuration
    - Track setup completion and policy updates
*/
...
BEGIN
    ...
    ELSE
        -- Handle the insert with proper conflict resolution
...
COMMENT ON FUNCTION handle_auth_user_creation IS 'Links Supabase Auth users to app profiles with proper roles, resolves ID conflicts safely';