/*
  # Fix Admin Authentication and Database Credentials

  1. Database Functions
    - Create functions to ensure admin user setup without conflicts
    - Handle auth user creation and linking
    - Create helper function to check admin status safely

  2. Security Policies
    - Update admin policies to use recursion-safe helper function
    - Ensure admin users have maximum credentials
    - Fix duplicate key constraint issues
...
INSERT INTO configuracion (key, valor) VALUES
    ...
    ('migration_fixed_duplicate_key', 'true')
ON CONFLICT (key) DO UPDATE SET 
    valor = EXCLUDED.valor,
    updated_at = NOW();
